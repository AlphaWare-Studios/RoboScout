...
public class QR : MonoBehaviour
{
    ...
    IEnumerator QRFallback(float Framerate)
    {
        if (hasScanned == false && Scan.activeSelf)
        {
            ...
            {
                ColorR[i] = new Color(Colors[i].r, Colors[i].r, Colors[i].r);
            }
            for (var i = 0; i < ColorG.Length; ++i)
            {
                ColorG[i] = new Color(Colors[i].g, Colors[i].g, Colors[i].g);
            }
            for (var i = 0; i < ColorB.Length; ++i)
            {
                ColorB[i] = new Color(Colors[i].b, Colors[i].b, Colors[i].b);
            }
            Scr.SetPixels(Colors);
            ...
            StartCoroutine(QRFallback(Framerate));
        }
    }
